...
info "Starting GNU-Linux native installation…"
...
curl -LO https://root.cern/download/root_v${ROOT_VERSION}.Linux-fedora32-x86_64-gcc10.1.tar.gz # Download ROOT precompiled for Linux Fedora 32 x86_64

tar xzf root_v${ROOT_VERSION}.Linux-fedora32-x86_64-gcc10.1.tar.gz -C "${TOOL_CHAIN_DIR}"
rm -f root_v${ROOT_VERSION}.Linux-fedora32-x86_64-gcc10.1.tar.gz
export ROOTSYS=""   
set +u
source "${TOOL_CHAIN_DIR}/root/bin/thisroot.sh"
set -u # BAT installer needs to find ROOT
ok "ROOT ${ROOT_VERSION} installed."


# ===== Geant4 installation (GNU-Linux + Qt5) =====
...
G4_BUILD_DIR="${TOOL_CHAIN_DIR}/geant4-${GEANT4_VERSION}-build"
...
# ---- Fedora system paths ----
XERCES_PREFIX="$(pkg-config --variable=prefix xerces-c 2>/dev/null || echo /usr)"
QT5_CMAKE_DIR="/usr/lib64/cmake/Qt5"
CMAKE_PREFIX_PATH="${ROOTSYS};${XERCES_PREFIX};${QT5_CMAKE_DIR}"
...
# ---- Build and Install ----
cmake --build "${G4_BUILD_DIR}" -j"$(nproc)"
cmake --install "${G4_BUILD_DIR}"

ok "Geant4 ${GEANT4_VERSION} installed successfully under ${G4_INSTALL_DIR}"
...
# throw() fix for C++11 compatibility
...
# Fix for ROOT::Math::Util::ToString issue in BCIntegrate.cxx
...
info "GeMSE_environment singularity is also available for use."
singularity build --tmpdir "${PREFIX}/images/tmp" gemse_env.simg docker://ramirezdiego/gemse_env:latest